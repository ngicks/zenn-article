...
型推論がかかるため`Seq ~`の部分を明示的に指定する必要があるのは引数なしで呼び出されたときのみです。
この場合明示的な指定が必要で、ないとエラーとなります。
...
破壊的変更と呼べる気がしますが、引数なしなのはエッジケースもいいところなので変更してしまうつもり。
...
- generic methods, やっぱあると便利です。
- method receiverのtype paramに型制約が必要な`Equal`とか`Collect`とかが実装できないとそこだけはトップレベルの関数を実装して使う今まで通りのスタイルとなります。一貫性が重要ならgeneric methodsは使わないほうがいいかも

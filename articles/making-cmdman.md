...
`Open`は[pty(7)](https://man7.org/linux/man-pages/man7/pty.7.html)に基づいて`/dev/ptmx`を開くことでptyのmasterとslaveを得ます（この用語まだ変わってないのか。）
...
single-forkでも実用上は困らないらしいんですが、[conmon](https://github.com/containers/conmon)(podman/CRI-Oのコンテナmonitor)はdouble-forkしてる。コメントはこう。
...
double-forkにした理由はcommit messageに書いた通り。

> Was single-fork detach
> Reason: prevent accidental acquirement of controlling terminal
> It only happens Solaris this day, or so I heard.
> But conmon does double-fork so we should follow it.

setsid一発だと、そのプロセス自身がsession leaderになります(`session == pid`)。session leaderはttyデバイスを`open`したときに**うっかり制御端末を取得してしまう**ことがある。今どきこれが実際に起きるのはSolarisぐらいらしいんですが、conmonがdouble-forkしてる以上それに倣っとくのが無難だろうと。

実装はこう。

https://github.com/ngicks/cmdman/blob/897a4ac18d5b53fd4943bfeeeda5a4d744415f0e/pkg/cmdman/mon_spawn_posix.go#L36-L113

- `SpawnMonitor`が自分自身を`__monitor`隠しコマンドとして再実行する。これが中間プロセス(intermediate)。
- 中間プロセスは`setsid`して新しいsessionのleaderになり、そこから本物のdaemonをもう一段fork。daemonはleaderではない(grandchild)ので、もう制御端末を取得できない。
- 中間プロセスは即exitするので、daemonはinitに里子に出される(reparent)。`envMonitorDaemon`環境変数でgrandchildだけがmonitor loopを直接走るようにマークしている。

不変条件(monitorは「自分がleaderでないsession」のメンバーである)は[e2eテスト](https://github.com/ngicks/cmdman/blob/897a4ac18d5b53fd4943bfeeeda5a4d744415f0e/e2e/cmdman/detach_test.go#L50-L81)で守ってます。single-forkだと`session == pid`になっちゃうので、これがそのままregression guardになる。

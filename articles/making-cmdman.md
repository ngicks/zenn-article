...
`cmdman`外から当然これらの状態を操作しえるため、それらのpaneやwidow意外にstateを保存すると容易にずれうることが考えられます。
...
- daemonize:
  - CreateProcessのdetachオプションへのリンク張る
- Unix Domain Socket:
  - サポート始まってること書く。時機も書く。リンク張る。Goも`net.Listern("unix", ...)`がwindowsで動くようになってっる。その時のGoのrelease noteへのリンク張る。
  - gRPCのurlがunix://でいいのか調べて、よいならこの行消す。だめなら、そこも変更いる旨かく
- pty / tty:
  - ConPTY使えばいいこと書く。
    - psmuxはConPTY使ってる
    - VSCodeもConPTY
  - [github.com/creack/pty]は未対応([#95](https://github.com/creack/pty/issues/95)、[#155](https://github.com/creack/pty/pull/155)、[#109](https://github.com/creack/pty/pull/109)、[#208](https://github.com/creack/pty/pull/208))。
  - 別のラッパー使う？例を二つあげる。
- process groupへのsignal:
  - なんかかけ
  - `cmdman stop`のgraceful exitは終了をIPCする仕組みを自前で組むか、Windowsに組み込まれた仕組みを使う。どうやったらいいのか書く。
- 0でsignalするpid生存確認:
  - これもなんか書く
- `cmdman events`でinotify:

ConPTYの話ちょっと補足。いつ追加された、その前までどうしてたか、vscodeを例に

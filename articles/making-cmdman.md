...
- 普通に`attach`を実装すると、detachした後にterminal stateが崩れてカーソルが表示されなかったりする
  - `claer`を実行すると戻る
- best effortで回復を実装した
- bubleteaはcmdmanより限られたせっとの回復しかしない
- podmanのコードを見てもどこにも回復が書かれてなくておかしいなと思って`podman container run ... watch ls -ls /tmp`に`podman attach`してdetachしてみると、terminal stateがこわれたままだった。実際実装されていなかっただけだった！
  - たぶんdockerでも同じ

### zellij / weztermに実装を広げるのが大変な理由

- per-window / per-paneでデータを格納する方法が
  - zellijには(今時点で)ないのでpluginの開発が多分必要
  - weztermはlua scriptの補助が必要でユーザー側のセットアップがデカすぎ

いろいろ詳しく書いてなんとなくこういうプラグイン？まで話を拡張して

### attach時に特定のシーケンスはリプレイしないとだめそう

- mux upとmux downを繰り返すとneovimでもtmuxのマウスコントロールが効いてる、
- tmuxのソース見たら特定シーケンスでマウス切ってそう？？
- シーケンスを貯めてattach時にリプレイするようにしたら

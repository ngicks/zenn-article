...
ただし[#38931](https://github.com/neovim/neovim/pull/38931)である通り、lockfileは古い内容だが、すでにrepositoryの状態が`version`で指定される最新状態であるとき、アップデートなしとみなされて、このdesync状態が解決できません。`XDG_CONFIG_HOME`がread-onlyとなるようなセットアップ・・・つまり筆者のように`nix`の`home-manager`を使っているとこの状態が容易に起きます。

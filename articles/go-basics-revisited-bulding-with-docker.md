...
- そもそもコンテナってなに
- コンテナランタイムである[Docker], [podman]のインストール手順
...
コンテナランタイムで有名なものに[Docker]があります。[podman]は[Docker]互換なので記事で述べられていることは基本的には[Docker]でも同様です。

## そもそもコンテナってなに

ざっくりコンテナとは何ぞやって言う概説のみ述べます。

### ざっくり使用する利点

コンテナはアプリをひとまとめにして隔離環境で動作させる仕組みのことです。

以下のようなことはよく起きます。なぜでしょう？: [Node.js]や[Python]でスクリプトを書いて自分の環境ではうまく動くことがわかりました。同僚にも使ってもらおうと、スクリプトファイルを渡すとうまく動かない。

大抵の場合は以下のような理由です。

- [Node.js], [Python]自体のバージョンが異なる
- スクリプトが依存しているアプリが存在しない
- アプリが依存しているライブラリが存在しない/バージョンが合わない
- 設定が異なる

などなど

[Python]などがどこでも動くスクリプトランタイムとして機能している一方で、それよりもっと外側の「環境」は受け渡すことができていなかったのです。

コンテナは、そういった環境ごとアプリをパッケージングすることができるものです。

コンテナを用いるとアプリそのもの、それに必要な外部コマンド、ライブラリ、設定ファイルなどをまとめてパッケージ化し、ビルドしたり配布したりすることができます。
...
コンテナを同僚に配布すれば動かないと苦情を受けることは基本的になくなるわけです。

さらに、そのアプリだけを切り出せたことで、以下が容易に行えます。

- アプリを複数の環境に持ち出せたり
- 同じアプリを複数動かせたり
- アプリの導入がホストに不要なライブラリを残さなくなったり
- アプリがホストの情報を読み取れなくなったり

します。

参考: https://docs.docker.com/get-started/docker-concepts/the-basics/what-is-a-container/
...
`--read-only`(`podman`では`--read-only-tmpfs=false`も)でコンテナを動作させましょう。
